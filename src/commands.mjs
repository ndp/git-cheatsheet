...
export const translations = {
  en: {
    locations: {
      ...
      docs: {
        ...
        remote_repo:
          "Versions of your project that are hosted on the Internet or network, ensuring all your changes are available for other developers. The default name is `origin`. Typical branches here: `master`, `shared-feature-x`, `release-y`",
      },
    },
    ...
    commands: {
      ...
      "merge x": {
        ...
        docs:
          "Merge changes from <branch name> into current branch.\rUse `&#8209;&#8209;no-commit` to leave" +
  ...
  fr: {
    locations: {
      ...
      docs: {
        stash:
          "Un endroit où remiser des modifications pendant que vous travaillez sur autre chose",
        workspace: "L'espace de travail local",
        index:
          "L'index (ou \"zone de transit\") maintient un instantané de l'espace de travail qui servira de base pour le prochain commit.",
        local_repo:
          "Un sous-répertoire nommé .git qui contient tous les fichiers nécessaires au référentiel. Branches typiques : `master`, `fonction-x`, `correctif-y`",
        remote_repo:
          "Versions de votre projet qui sont hébergés sur le réseau ou Internet, pour mettre toutes vos modifications à la disposition d'autres développeurs. Par défaut «origin». Branches typiques : `master`, `fonction-x-partagée`, `version-y`",
      },
    },
    ...
    commands: {
      ...
      "clone x": {
        ...
        docs: "Télécharge le DÉPÔT_DISTANT et se positionne sur la TÊTE de sa branche master.",
      },
      "pull x x": {
      ...
      "reset --hard x/x": {
        ...
        docs: "Réinitialise l'ESPACE_DE_TRAVAIL et le DÉPÔT_LOCAL pour les faire correspondre à la BRANCHE du DÉPÔT_DISTANT. Utilisez 'git reset --hard origin/master' pour rejeter tous les commits du DÉPÔT_LOCAL. Utilisez ceci pour reprendre après une fusion qui a échoué.",
      },
      "fetch x x": {
  ...
  "zh-cn": {
    locations: {
      ...
      docs: {
        ...
        local_repo:
          ".git 文件夹保存版本库需要的全部信息(Git 版本库的骨架)，一般包括分支<strong>master</strong>, <strong>feature-x</strong>, <strong>bugfix-y</strong>",
        remote_repo:
          '在网络（局域或因特网）上共享给其他开发者的版本库，一般叫"origin". 一般包括分支<strong>master</strong>, <strong>shared-feature-x</strong>, <strong>release-y</strong>',
      },
    },
    ...
    commands: {
      ...
      "clone x": {
        cmd: "clone <repo>",
        docs: "下载<repo>指定的版本库，并在工作区迁出master分支的HEAD版本",
      },
      "pull x x": {
        cmd: "pull <remote> <refspec>",
      ...
      "reset --hard x/x": {
        cmd: "reset --hard <remote>/<branch>",
        docs: "重置本地版本库，让它与远程版本一致；用 `reset &#8209;&#8209;hard origin/master` 来丢弃所有的本地改动；用这个来处理失败的合并，直接从远程开始",
      },
      "fetch x x": {
        cmd: "fetch <remote> <refspec>",
  ...
  es: {
    locations: {
      ...
      docs: {
        ...
        index:
          'El index (o "staging area") contiene una captura del contenido del árbol de trabajo. Esta captura representa a los contenidos del próximo commit.',
        local_repo:
          "Un subdirectorio llamado .git que contiene todos los archivos necesarios — un esqueleto del repositorio Git. Ramas típicas: `master`, `feature-x`, `bugfix-y`",
        remote_repo:
          'Version(es) del proyecto que están alojadas en Internet o una red, asegurando que todos los cambios están disponibles para otros desarrolladores. Por defecto es "origin". Ramas típicas aquí son: `master`, `shared-feature-x`, `release-y`',
      },
    },
    ...
    commands: {
      ...
      "clone x": {
        cmd: "clone <repo>",
        docs: "Descarga el repositorio especificado por <repo> y posiciona el `HEAD` en la rama master.",
      },
      "pull x x": {
        cmd: "pull <remote> <refspec>",
      ...
      "reset --hard x/x": {
        cmd: "reset --hard <remote>/<branch>",
        docs: "Equipara el espacio de trabajo y el index con una rama remota. Usar `reset &#8209;&#8209;hard origin/master` para descartar todos los commits en la rama local master. Se puede utilizar para comenzar de nuevo desde una combinación/merge fallida.",
      },
      "fetch x x": {
        cmd: "fetch <remote> <refspec>",
  ...
  de: {
    locations: {
      ...
      docs: {
        stash:
          'Hier können Änderungen "geparkt" werden, während man an etwas anderem arbeitet',
        workspace: "Checkout des lokalen Repositories",
        index:
          'Zu commitende Dateien. Vor einem "commit" (checkin), müssen die Dateien zuerst zu dem Index hinzugefügt werden. Wird auch "staging area" oder "staged files" genannt.',
        local_repo:
          "Der lokale clone des Remote Repositories. Es wird in dem Verzeichnis `.git` verwaltet. Typische Branches sind: `master`, `feature-x`, `bugfix-y`",
        remote_repo:
          "Der Ursprung des lokalen Repositories. Durch ein `clone` wird es automatisch als `origin` gesetzt.",
      },
    },
  ...
  ko: {
    locations: {
      ...
      docs: {
        ...
        workspace: "로컬 체크 아웃",
        index:
          '커밋하기를 원하는 파일. 파일을 "commit"(checkin) 하기 전에 먼저 Index에 추가하여야 합니다. "현재 디렉토리 캐시", "스테이징 영역", "캐시", 스태이지 파일" 으로 불리웁니다.',
        local_repo:
          "필요한 저장소 파일들을 모두 포함하고 있는 `.git`이라는 서브 디렉토리 (Git 저장소 뼈대를 말한다). 일반적인 브런치: `master`, `feature-x`, `bugfix-y`",
        remote_repo:
          "인터넷이나 네트워크로 호스팅되는 프로젝트 버전으로, 모든 변경 사항을 다른 개발자들과 공유하여 사용할 수 있습니다. 기본 이름은 `origin`. 일반적인 브런치: `master`, `shared-feature-x`, `release-y`",
      },
    },
    commands: {
      status: {
  ...
  it: {
    locations: {
      ...
      docs: {
        stash:
          "Uno spazio in cui conservare le modifiche mentre si lavora a qualcos'altro.",
        workspace:
          'Lo spazio di lavoro locale (la cartella nella quale si lavora). Spesso è noto anche come "working tree".',
        index:
          'Uno spazio che contiene i file pronti per essere committati. Spesso è noto anche come "current directory cache", "cache" o "staging area". I file al suo interno spesso sono noti anche come "staged files".',
        local_repo:
          "Uno spazio che contiene tutti i file committati e le informazioni sui branch. Si presenta come una sotto-cartella denominata `.git`. I nomi tipici per i branch usati qui sono: `master`, `feature-x`, `bugfix-y`.",
        remote_repo:
          "Uno spazio online che contiene tutte le varie versioni del progetto, accessibile anche agli altri sviluppatori. Il nome usato per identificare un repository remoto di solito è `origin`. I nomi tipici per i branch usati qui sono: `master`, `shared-feature-x`, `release-y`.",
      },
    },
    commands: {
      status: {
      ...
      "merge x": {
        cmd: "merge <commit or branch>",
        docs:
          "Unisce i cambiamenti di <branch> o di <commit> nel branch corrente.\rSpecificare `&#8209;&#8209;no-commit` per non eseguire" +
